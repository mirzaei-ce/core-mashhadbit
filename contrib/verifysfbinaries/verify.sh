...
###   This script attempts to download the signature file SHA256SUMS.asc from mashhadbit.org
...
WORKINGDIR="/tmp/mashhadbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://mashhadbit.org/bin/"
VERSIONPREFIX="mashhadbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'mashhadbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Mashhadbit Core binary release signing key installed?"
   fi
